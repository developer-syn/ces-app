<div class="space-y-4">
    ...
    <div>
        ...
        <input type="text" name="address" id="address" required
            value="{{ old('address', $schoolInfo->address ?? '') }}"
            class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 transition duration-150 @error('address') border-red-500 @enderror">
        @error('address')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
    </div>
        ...
        <div class="flex items-center gap-4">
            ...
            @if (!empty($schoolInfo->logo_path))
                <div class="shrink-0 relative group">
                    ...
                    <div
                        class="absolute inset-0 bg-black bg-opacity-50 hidden group-hover:flex items-center justify-center rounded-lg">
