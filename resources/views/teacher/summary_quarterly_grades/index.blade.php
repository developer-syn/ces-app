                ...
                <div class="mx-auto sm:px-6 lg:px-8">
                    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                        ...
                        <div class="px-6 py-4 border-b border-gray-200 bg-white">
                            <form method="GET" action="{{ route('teacher.summary_quarterly_grades.index') }}" class="space-y-4 md:space-y-0 md:grid md:grid-cols-4 md:gap-6">
                                <div class="flex flex-col">
                                    <label for="year_level_id" class="block text-sm font-medium text-gray-700 mb-1">Year Level</label>
                                    <select name="year_level_id" id="year_level_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                        <option value="">All Year Levels</option>
                                        @foreach($yearLevels as $yearLevel)
                                            <option value="{{ $yearLevel->id }}" {{ request('year_level_id') == $yearLevel->id ? 'selected' : '' }}>
                                                {{ $yearLevel->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <select name="section" id="section" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                ...
                                <div class="flex flex-col">
                                    <label for="user_id" class="block text-sm font-medium text-gray-700 mb-1">Teacher</label>
                                    <select name="user_id" id="user_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                ...
                                <div class="flex flex-col">
                                    <label for="quarter_id" class="block text-sm font-medium text-gray-700 mb-1">Quarter</label>
                                    <select name="quarter_id" id="quarter_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                        <option value="">All Quarters</option>
                                        @foreach($quarters as $quarter)
                                            <option value="{{ $quarter->id }}" {{ request('quarter_id') == $quarter->id ? 'selected' : '' }}>
                                                {{ $quarter->name }}
                                            </option>
                                        @endforeach
                                    </select>
                ...
                <div class="overflow-x-auto relative" x-data="{ loading: true }" x-init="setTimeout(() => loading = false, 500)">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <thead class="bg-gray-50 sticky top-0">
                            <tr>
                                <th scope="col" class="px-6 py-3 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider sticky left-0 bg-gray-50 shadow-sm z-10">
                                    Student Name
                                </th>
                                @php
                                    $subjectsList = ['English', 'Filipino', 'Mathematics','Science', 'Araling Panlipunan', 'EsP', 'EEP', 'MAPEH', 'Music', 'Art', 'Physical Education', 'Health'];
                                @endphp
                                ...
                                @endforeach
                                <th scope="col" class="px-6 py-3 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider bg-blue-50">
                                    Average
                                </th>
                            </tr>
                        </thead>
                                    ...
                                    @foreach ($subjectsList as $subject)
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-center font-bold bg-gray-100">
                                                <div class="text-sm text-gray-900">
                                                    @if ($mapehAverage !== null)
                                                        <span class="@if ($mapehAverage < 75) text-red-600 @elseif($mapehAverage >= 90) text-green-600 @endif">
                                        ...
                                        @else
                                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                                <div class="text-sm text-gray-900">
                                                    @if ($grade !== null)
                                                        <span class="@if ($grade < 75) text-red-600 @elseif($grade >= 90) text-green-600 @endif">
                                                            {{ number_format($grade) }}
                                                        </span>
                                                    @else
                                                        <span class="text-gray-400">N/A</span>
                                        ...
                                        <div class="text-sm font-semibold text-gray-900 text-center">
                                            @php
                                                ...
                                                $filteredGrades = $student->classRecords
                                                    ->filter(fn($record) => !in_array($record->subject->name, $mapehSubjects))
                                                    ->pluck('quarterly_grade');

                                                // Merge MAPEH average into overall grades
                                                $allGrades = $filteredGrades->merge($mapehAverage !== null ? [$mapehAverage] : []);
