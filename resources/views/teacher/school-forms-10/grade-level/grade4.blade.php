{{-- teacher.school-forms-10.grade4.blade.php --}}
@if (isset($enrollment) && $enrollment->yearLevel->id === 4)
        ...
        <div class="p-2 border border-gray-800">
            <div class="grid grid-cols-1 gap-1">
                <div class="flex items-center gap-2">
                    ...
                    <span class="border-b border-gray-800 flex-1">{{ $student->schoolInfo->school_name }}</span>
                    <span class="font-semibold ml-4">School ID:</span>
                    <span class="border-b border-gray-800 w-32">{{ $student->schoolInfo->school_id }}</span>
                </div>

                <div class="flex items-center gap-2">
                    <span class="font-semibold w-12">District:</span>
                    <span class="border-b border-gray-800 w-36">{{ $student->schoolInfo->district }}</span>
                    <span class="font-semibold ml-1">Division:</span>
                    <span class="border-b border-gray-800 w-36">{{ $student->schoolInfo->division }}</span>
                    <span class="font-semibold ml-4">Region:</span>
                    <span class="border-b border-gray-800 w-24">{{ $student->schoolInfo->region }}</span>
                </div>

                <div class="flex items-center gap-2">
                    <span class="font-semibold">Classified as Grade:</span>
                    <span class="border-b border-gray-800 w-16 text-center">{{ $enrollment->yearLevel->name }}</span>
                    <span class="font-semibold ml-4">Section:</span>
                    <span class="border-b border-gray-800 w-16">{{ $enrollment->teacher->section }}</span>
                    <span class="font-semibold ml-4">School Year:</span>
                    <span class="border-b border-gray-800">{{ $enrollment->schoolYear->name }}</span>
                </div>

                <div class="flex items-center gap-2">
                    <span class="font-semibold">Name of Adviser/Teacher:</span>
                    <span class="border-b border-gray-800 flex-1">{{ $enrollment->teacher->name }}</span>
        ...
        <table class="w-full border-collapse border border-gray-800" style="height: 450px;">
            ...
            <tbody>
                ...
                @foreach ($subjects as $subject)
                    ...
                    @if ($subject === 'MAPEH')
                        <tr>
                            ...
                            @for ($q = 1; $q <= 4; $q++)
                                <td class="border border-gray-800 p-1 text-center">{{ $mapehAverages[$q] ?? '' }}
                                </td>
                            @endfor
                ...
                <tr>
                    ...
                    @for ($q = 1; $q <= 4; $q++)
                        <td class="border border-gray-800 p-1 text-center font-bold">{{ $quarterlyAverages[$q] }}
                        </td>
                    @endfor
...
@else
    ...
    <div class="overflow-x-auto">
        ...
        <div class="border-1 border-gray-800 overflow-hidden">
            <table class="w-full border-collapse border border-gray-800" style="height: 372px;">
                <thead>
                    <tr>
                        <th class="border border-gray-800 p-2 text-left" rowspan="2">
                            Learning
        ...
        <div class="border-1 border-gray-800 overflow-hidden">
            <table class="w-full">
                <thead>
                    <tr>
                        <td class="border border-gray-800 px-4 p-3" style="width: 30%">Remedial Classes</td>
                        <td colspan="6" class="border border-gray-800 px-2 p-3">Date Conducted: ______________
                            to ______________</td>
                    </tr>
                    <tr>
                        <th class="border border-gray-800 p-1">Learning Areas</th>
                        <th class="border border-gray-800 p-1" style="width: 10%;">Final Rating</th>
